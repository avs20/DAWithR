...
ggcorr(data[, 2:12], geom="blank", label=TRUE, label_size=3,label_round=2,
      hjust=0.7,color = "grey50",layout.exp = 3)

```

For a quick overview of the data I plotted a scatterplot matrix of the variables
to look at the correlation and the distribution with other variables. 
...
The scatter plot also shows many outliers here and there so I need to remove the
outliers before further processing.
...
From the above plot matrix a relationship between density and residual sugar 
looks linear. Plotting them again 
...
at how the values of different features spread with quality
...
These box plots give us an average idea of what features change according to 
quality. Some specific mentions are
...
I am still interested in residual sugar due to its bimodal histogram on the 
log axis.So instead of quality lets look at the scatter plots of residual sugar
with other features.
...
p1<-qplot(data = data, x = factor(quality), y = clean.chlorides)+
  ...
  stat_summary(fun.y=mean, color="red", hape=18,geom="point", size=3,
              show_guide = TRUE)
...
It looks like a negative correlation between pH and **fixed.acidity** .
The above plot also shows a linear regression line. 
...
ggplot(data=data,aes(x=clean.totol.sulfur.dioxide, residual.sugar))+
  geom_point(position="jitter", alpha=0.5, size=0.75) +
  xlab("clean total sulfur dioxide")
...
ggplot(data = data, aes(y = density, x= residual.sugar, color=quality.cat) ) +
